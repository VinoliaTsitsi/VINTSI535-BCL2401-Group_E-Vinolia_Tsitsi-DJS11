...
const Show = () => {
  ...
  const [showSeasons, setShowSeasons] = useState(false); // State to toggle season display
  ...
  const handleShowBlockClick = () => {
    setShowSeasons(!showSeasons);
  };

  const handleSeasonSelect = (seasonNumber) => {
    setSelectedSeason(seasonNumber);
    setOpenDropdown(seasonNumber);
  ...
  return (
    <div className="show-container">
      ...
      <div className="show-header" onClick={handleShowBlockClick}>
        <h1 className="show-title">{show.title}</h1>
        <img src={show.image} alt={show.title} className="show-image" />
        <p className="show-description">{show.description}</p>
        {showSeasons && (
          <p className="seasons-info">Number of Seasons: {show.seasons.length}</p>
        )}
      </div>
